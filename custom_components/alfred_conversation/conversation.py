"""Conversation platform for ALFRED."""
...
async def async_setup_entry(
    ...
    agent = AlfredConversationEntity(config_entry)
...
class AlfredConversationEntity(
    conversation.ConversationEntity, conversation.AbstractConversationAgent
):
    """ALFRED conversation agent."""

    _attr_has_entity_name = True
    _attr_name = None

    def __init__(self, entry: AlfredConfigEntry) -> None:
    ...
    async def async_process(
        ...
        if llm_api:
            system_parts.append(llm_api.api_prompt)
        ...
        for _iteration in range(MAX_TOOL_ITERATIONS):
            try:
                ...
                LOGGER.error("Error calling ALFRED: %s", err)
                intent_response.async_set_error(
                    intent.IntentResponseErrorCode.UNKNOWN,
                    f"Sorry, I had a problem talking to ALFRED: {err}",
                )
                return conversation.ConversationResult(
            ...
            if not tool_calls_list or not llm_api:
                break
